...
const compilations = ['client/tsconfig.json', 'server/tsconfig.json'];
...
const tasks = compilations.map(function (tsconfigFile) {
  ...
  function createPipeline(build, emitError, transpileOnly) {
    ...
    const compilation = tsb.create(
      absolutePath,
      overrideOptions,
      { verbose: false, transpileOnly, transpileOnlyIncludesDts: transpileOnly, transpileWithSwc: true },
  ...
  const watchTask = task.define(
    ...
    task.series(cleanTask, () => {
      ...
      return watchInput.pipe(util.incremental(pipeline, input)).pipe(gulp.dest(out));
    })
  );
...
const compileExtensionMediaBuildTask = task.define('compile-extension-media-build', () =>
...
gulp.task(
  task.define('extensions-ci', task.series(compileNonNativeExtensionsBuildTask, compileExtensionMediaBuildTask))
);
...
gulp.task(
  task.define('extensions-ci-pr', task.series(compileExtensionsBuildPullRequestTask, compileExtensionMediaBuildTask))
);
