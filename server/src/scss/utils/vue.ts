...
export function getVueSCSSContent(
	content: string,
	regions = getVueSCSSRegions(content)
) {
...
function convertVueTextDocument(document: TextDocument, regions: Region[]) {
	return TextDocument.create(
		...
		'scss',
		document.version,
		getVueSCSSContent(document.getText(), regions)
	);
}
...
export function getSCSSRegionsDocument(
	...
	if (
		vueSCSSRegions.some((region) => region[0] <= offset && region[1] >= offset)
	) {
		return {
			document: convertVueTextDocument(document, vueSCSSRegions),
			offset,
		};
	}
